...
function runSurvival(canvas, statusEl, durationSec, hard, done) {
  ...
  function loop(ts) {
    ...
    for (const h of hazards) h.y += (h.vy * dt) / 16;
    ...
    for (let x = 0; x < W; x += 40) ctx.fillRect(x, groundY, 20, 4);
...
export default function SurvivalGame({ onDone }) {
  const canvasRef = useRef(null);
  const statusRef = useRef(null);
  const cleanupRef = useRef(null);

  useEffect(() => {
    if (!canvasRef.current || !statusRef.current) return;
    const stop = runSurvival(canvasRef.current, statusRef.current, 10, true, onDone);
  ...
  return (
    <>
      <canvas
        ref={canvasRef}
        id="platform-canvas-hard"
        width={720}
        height={280}
        aria-label="Simulation pare-feu"
      />
      <p ref={statusRef} className="platform-status" />
    </>
  );
}
